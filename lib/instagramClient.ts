...
async function getIGInsights(mediaId: string, token: string, forceRefresh = false): Promise<IGInsights> {
  ...
  const url = `https://graph.instagram.com/v21.0/${mediaId}/insights?metric=${metrics}&access_token=${token}`
...
export async function getFollowerCount(): Promise<number> {
  ...
  if (!creds) return 0
  const { token, userId } = creds
  const url = `https://graph.instagram.com/v21.0/${userId}?fields=followers_count&access_token=${token}`
  const res = await fetch(url)
  const data = await res.json()
...
export async function getIGMediaUrl(mediaId: string): Promise<string> {
  ...
  const { token } = creds
  const url = `https://graph.instagram.com/v21.0/${mediaId}?fields=media_url&access_token=${token}`
  const res = await fetch(url)
  const data = await res.json()
  if (data.error) throw new Error(`IG API: ${data.error.message}`)
